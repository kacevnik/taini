<?php
function kdv_redirect(){
    $url = substr($_SERVER['REQUEST_URI'], 1);
    if(!preg_match('/\/page\//i', substr($_SERVER['REQUEST_URI'], 1), $out)){
        $red = preg_replace('/\/[\w-]+\//i', '/', substr($_SERVER['REQUEST_URI'], 1));
        if($url != $red){
            print_r($out);
            header('Location: '.bloginfo('url').'/'.$red);
            exit();
        }
    }
}
...
$options = array (
	...
	array("name" => "Иконки социальных сетей",
		...
		"std" => "true",
		"type" => "checkbox"),
    array("name" => "Включить новогоднее настроение",
        "desc" => "Включить на сайте новогоднее настроение?(15.12.xx - 15.01.xx)",
        "id" => $shortname."_kdv_hny",
        "std" => "true",
        "type" => "checkbox"),   
...
function kdv_hny($in){
    $date = array('1512','1612','1712','1812','1912','2012','2112','2212','2312','2412','2512','2612','2712','2812','2912','3012','3112','0101','0201','0301','0401','0501','0601','0701','0801','0901','1001','1101','1201','1301','1401','1501');
    if(in_array(current_time('dm'), $date)){
        if($in){
            echo ' hny';
        }
    }
